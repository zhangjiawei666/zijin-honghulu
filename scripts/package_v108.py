# -*- coding: utf-8 -*-
"""v1.0.8 打包脚本：同步 win-unpacked → 版本目录 → ZIP（排除实验遗留 node-pty）"""
...
# 0. 清理实验遗留的 node-pty（发布版不需要）
pty_dir = os.path.join(APP_DIR, "node-pty")
if os.path.exists(pty_dir):
    shutil.rmtree(pty_dir)
    print("[clean] 移除实验遗留 node-pty")
...
# 2. 版本目录（从 win-unpacked 全量复制，排除 node-pty）
