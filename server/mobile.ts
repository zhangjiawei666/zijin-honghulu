...
import { fetchRealtimeQuotes, fetchStockNames, monitor } from './monitor.js';
...
function toMarketCode(input: string) {
  ...
  try {
    ...
    res.json({
      quotes: items.map((item) => {
        const quote = quoteMap.get(toMarketCode(item.code));
...
app.post('/api/watchlist/batch', async (req, res) => {
  try {
    ...
    const nameMap = await fetchStockNames(parsed.filter((item) => !item.name).map((item) => item.code)).catch(() => new Map<string, string>());
    const now = new Date().toISOString();
    const addedCount = db.addWatchItemsBulk(parsed.map((item) => ({ id: uuidv4(), code: item.code, name: item.name || nameMap.get(item.code) || item.code, created_at: now })));
...
app.delete('/api/watchlist/:id', (req, res) => {
  if (!db.deleteWatchItem(req.params.id)) return res.status(404).json({ error: '自选股不存在' });
  res.json({ success: true });
});

app.post('/api/monitor/run', async (_req, res) => {
  try { res.json(await monitor.runCheck('manual', { builtinOnly: true })); }
  catch (error: any) { res.status(500).json({ error: error?.message || '巡检失败' }); }
});
...
const distDir = path.join(appDir, '..', 'dist-mobile');
if (fs.existsSync(distDir)) {
  app.use(express.static(distDir));
  app.get(/^\/(?!api(\/|$)).*/, (_req, res) => res.sendFile(path.join(distDir, 'index.html')));
}

process.env.STOCK_MONITOR_BUILTIN_ONLY = '1';

app.listen(port, '0.0.0.0', () => {
  console.log(`[Mobile] 服务已启动：http://0.0.0.0:${port}`);
  monitor.start();
});
