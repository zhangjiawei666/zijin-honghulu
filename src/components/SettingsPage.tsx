...
/** 一键登录流程阶段 */
type LoginPhase = 'idle' | 'starting' | 'waiting_auth' | 'success' | 'error';
...
export function SettingsPage({ 
  ...
  /** 用系统默认浏览器打开授权链接（桌面版走 IPC，浏览器开发模式走 window.open） */
  const openAuthUrlInBrowser = useCallback(async (url: string) => {
    const shellBridge = (window as any).monitorShell;
    if (shellBridge?.openExternal) {
      try {
        const result = await shellBridge.openExternal(url);
        if (!result?.ok) {
          console.error('打开浏览器失败，回退 window.open:', result?.error);
  ...
  /** 一键登录：SSE 流式接收 auth_url → 打开浏览器 → 等待授权结果 */
  const startOneClickLogin = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        throw new Error(`登录服务不可用（${response.status}）`);
      }

      const reader = response.body.getReader();
      const decoder = new TextDecoder();
      let buffer = '';
      let sawResult = false; // 是否收到 success / error 终态事件
      ...
      while (true) {
        ...
        for (const chunk of chunks) {
          ...
          if (event.type === 'auth_url' && event.url) {
            ...
            sawResult = true;
            setLoginPhase('error');
            setLoginError(event.message || '登录失败，请重试');
            MessagePlugin.error(event.message || '登录失败，请重试');
          }
        }
      }

      // 流结束但没有收到 success/error
      if (!sawResult && !sawSuccess) {
        setLoginPhase((prev) => (prev === 'waiting_auth' || prev === 'starting' ? 'error' : prev));
        setLoginError((prev) => prev || '登录连接已断开，请重试');
      }
    } catch (error: any) {
      setLoginPhase('error');
  ...
  }, [checkLoginStatus, openAuthUrlInBrowser]);
  ...
  const handleLogout = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        MessagePlugin.error(data.error || '退出登录失败');
      }
    } catch (error: any) {
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              >
                登录 CodeBuddy 账号以使用 AI 分析能力（产业链分析、买点研判等）
              </p>
            </div>
            <Button
              variant="text"
          ...
          <div className="flex items-center gap-3 mb-4">
            {loginStatus.checking ? (
              ...
              <>
                ...
                <Tag size="small" variant="outline" color="success">
                  {loginStatus.method === 'env' ? '环境变量' : 'CodeBuddy 账号'}
                </Tag>
                {loginStatus.user?.userNickname && (
                  <span
                    className="text-sm"
                    style={{ color: 'var(--td-text-color-secondary)' }}
                  >
          ...
          {/* 一键登录流程 */}
          {!loginStatus.isLoggedIn && !loginStatus.checking && (
            <div
            ...
            >
              {loginPhase === 'idle' || loginPhase === 'error' ? (
                <div className="flex items-center gap-3">
                  <Button
                    theme="primary"
                    onClick={startOneClickLogin}
                  >
                    {loginPhase === 'error' ? '重新登录' : '一键登录'}
                  </Button>
                  <span
                    className="text-sm"
                    style={{ color: 'var(--td-text-color-placeholder)' }}
                  >
                    点击后将打开浏览器完成授权，无需任何命令行操作
                  </span>
                </div>
              ) : loginPhase === 'starting' ? (
                <div className="flex items-center gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <div
                    ...
                    style={{ color: 'var(--td-text-color-placeholder)' }}
                  >
                    已在系统浏览器中打开授权页面，请点击「授权」。完成后此页面会自动更新。
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="small"
                      onClick={() => openAuthUrlInBrowser(authUrl)}
                      disabled={!authUrl}
                    >
                      重新打开授权页面
                    </Button>
          ...
          <div>
            <div
              ...
              style={{ color: 'var(--td-text-color-placeholder)' }}
            >
              推荐使用上方「一键登录」：点击后在浏览器中授权一次，之后打开应用即可直接使用。
            </div>
          </div>
